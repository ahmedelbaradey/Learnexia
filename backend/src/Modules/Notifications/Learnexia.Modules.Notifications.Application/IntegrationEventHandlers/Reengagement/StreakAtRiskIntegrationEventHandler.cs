...
/// <summary>
/// Consumes <see cref="StreakAtRiskIntegrationEvent"/> (published by <c>StreakAtRiskJob</c> at 18:00 UTC)
/// and dispatches a <c>StreakAtRisk</c> nudge ("6 hours left — one lesson keeps your streak alive!")
/// (P4-09 B4-5 / AC1 + AC2). EF access lifted to services (Option-C rule).
/// </summary>
public sealed class StreakAtRiskIntegrationEventHandler
    : INotificationHandler<StreakAtRiskIntegrationEvent>
{
    ...
    public StreakAtRiskIntegrationEventHandler(
    ...
    public async Task Handle(StreakAtRiskIntegrationEvent ev, CancellationToken ct)
    {
        try
        {
            ...
            if (parentId is null)
            {
                _logger.LogInfo($"P4-09: StreakAtRiskHandler — no parent for studentId={ev.StudentId}; skip.");
                return;
            }

            var category = NotificationCategory.StreakAtRisk;
            const string code = "STREAK_AT_RISK";
        ...
        {
            _logger.LogError(ex, $"P4-09: StreakAtRiskIntegrationEventHandler threw for studentId={ev.StudentId}.");
        }
    }
}
