...
/// <summary>
/// Consumes <see cref="BadgeEarnedIntegrationEvent"/> and dispatches an <c>Achievement</c>
/// nudge ("You earned the {badgeCode} badge!") (P4-09 B4-5 / AC2).
/// The badge code is included in the copy placeholders (no PII).
/// </summary>
public sealed class BadgeEarnedIntegrationEventHandler
    : INotificationHandler<BadgeEarnedIntegrationEvent>
{
    ...
    public BadgeEarnedIntegrationEventHandler(
    ...
    public async Task Handle(BadgeEarnedIntegrationEvent ev, CancellationToken ct)
    {
        try
        {
            ...
            if (parentId is null)
            {
                _logger.LogInfo($"P4-09: BadgeEarnedHandler — no parent for studentId={ev.StudentId}; skip.");
                return;
            }

            var category = NotificationCategory.Achievement;
            const string code = "BADGE_EARNED";
        ...
        {
            _logger.LogError(ex, $"P4-09: BadgeEarnedIntegrationEventHandler threw for studentId={ev.StudentId}.");
        }
    }
}
