    ...
    : INotificationHandler<MissionCompletedIntegrationEvent>
{
    ...
    public async Task Handle(MissionCompletedIntegrationEvent ev, CancellationToken ct)
    {
        try
        {
            ...
            if (parentId is null)
            {
                _logger.LogInfo($"P4-09: MissionCompletedHandler — no parent for studentId={ev.StudentId}; skip.");
                return;
            }
        ...
        {
            _logger.LogError(ex, $"P4-09: MissionCompletedIntegrationEventHandler threw for studentId={ev.StudentId}.");
        }
    }
}
