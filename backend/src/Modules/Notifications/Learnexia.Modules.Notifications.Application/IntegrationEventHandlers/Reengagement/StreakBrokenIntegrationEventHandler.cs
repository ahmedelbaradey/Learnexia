...
/// <summary>
/// Consumes <see cref="StreakBrokenIntegrationEvent"/> and dispatches a <c>StreakAtRisk</c>
/// nudge ("Start a new streak!") via <see cref="INudgeDispatcher"/> (P4-09 B4-5 / AC2).
///
/// Flow: resolve parent → load prefs (default if absent) → count sent-today → evaluate eligibility
/// → Redis SETNX dedupe → render copy → dispatch. Each step is individually logged; the entire
/// handler is wrapped in a try/catch per ADR 0002 §3 (fail-soft).
/// </summary>
    ...
    : INotificationHandler<StreakBrokenIntegrationEvent>
{
    ...
    public StreakBrokenIntegrationEventHandler(
    ...
    public async Task Handle(StreakBrokenIntegrationEvent ev, CancellationToken ct)
    {
        try
        {
            ...
            if (parentId is null)
            {
                _logger.LogInfo($"P4-09: StreakBrokenHandler — no parent for studentId={ev.StudentId}; skip.");
                return;
            }

            var category = NotificationCategory.StreakAtRisk;
            const string code = "STREAK_BROKEN";
