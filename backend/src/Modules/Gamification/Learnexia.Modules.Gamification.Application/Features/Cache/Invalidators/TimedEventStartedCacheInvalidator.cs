...
/// <summary>
/// Cache invalidator for <see cref="TimedEventStartedDomainEvent"/> — P4-11-B1-B.
///
/// When a timed event activates (post-commit per ADR 0002 §2):
///   DEL <c>gamification:timed_events:active</c> — forces the next read to repopulate from Postgres
///   so the newly-active event is returned to the dashboard banner and the XP boost calculator.
///
/// Fail-soft: the outer try/catch ensures no exception propagates (ADR 0002 §3).
/// </summary>
internal sealed class TimedEventStartedCacheInvalidator
    : INotificationHandler<TimedEventStartedDomainEvent>
{
    ...
    public TimedEventStartedCacheInvalidator(
    ...
    public async Task Handle(TimedEventStartedDomainEvent notification, CancellationToken ct)
    {
        ...
        catch (Exception)
        {
            ...
            _logger.LogWarn(
                "P4-11: TimedEventStartedCacheInvalidator failed — Postgres is authoritative, nightly rebuild will reconcile.");
        }
    }
}
