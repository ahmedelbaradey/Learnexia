...
/// <summary>
/// Cache invalidator for <see cref="StreakBrokenDomainEvent"/> — P4-10 Batch 2.
///
/// On every streak break (post-commit per ADR 0002 §2):
///   DEL <c>gamification:student:{id}:streak</c> — forces the next read to repopulate from Postgres
///   with the reset <c>CurrentStreak = 0</c> value.
///
/// Fail-soft: the outer try/catch ensures no exception propagates (ADR 0002 §3).
/// </summary>
internal sealed class StreakBrokenCacheInvalidator : INotificationHandler<StreakBrokenDomainEvent>
{
    private readonly IGamificationCache _cache;
    private readonly GamificationCacheOptions _opts;
    private readonly ILoggerManager _logger;

    public StreakBrokenCacheInvalidator(
    ...
    public async Task Handle(StreakBrokenDomainEvent notification, CancellationToken ct)
    {
        ...
        catch (Exception)
        {
            ...
            _logger.LogWarn(
                "P4-10: StreakBrokenCacheInvalidator failed — Postgres is authoritative, nightly rebuild will reconcile.");
        }
    }
}
