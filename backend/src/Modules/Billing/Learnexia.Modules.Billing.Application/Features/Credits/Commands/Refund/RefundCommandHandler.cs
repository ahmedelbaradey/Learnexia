...
public class RefundCommandHandler : BaseResponseHandler, ICommandHandler<RefundCommand, BaseResponse<DebitResultDto>>
{
    ...
    public async Task<BaseResponse<DebitResultDto>> Handle(RefundCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var creditTransaction = account.Refund(request.Amount, request.IdempotencyKey, request.RelatedPaymentId);
        ...
        {
            _logger.LogError(ex, $"Error in RefundCommand for childId={request.ChildId}");
