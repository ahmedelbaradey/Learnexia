...
/// <summary>
/// SSE endpoint for the AI Helper "Explain" intent (P3-04 BE-4).
///
...
public sealed class ExplainController : ControllerBase
{
    ...
    /// <summary>
    /// Explain a concept/lesson/skill for the authenticated student (AI Helper intent #1).
    ///
    /// Streams the safety-approved explanation as Server-Sent Events.
    /// Grade, age, and language are resolved from the student's JWT — never from the request body.
    /// </summary>
    /// <param name="request">Command body (LessonId/ConceptId/SkillId + optional Question).</param>
    ...
    /// </remarks>
    [HttpPost("Explain")]
    public async Task Explain([FromBody] ExplainConceptCommand request, CancellationToken ct)
    {
        ...
        catch (FluentValidation.ValidationException vex)
        {
            ...
            // Errors are already localised by ExplainConceptCommandValidator.
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Unhandled exception in ExplainController — student-facing error suppressed");
        ...
        switch (result)
        {
            case ExplainResult.Streamed streamed:
                // Safety-approved content — buffer was already safety-filtered by ISafetyLayer.
                // Emit as one or a few message frames (the buffer is pre-checked — no raw streaming).
            ...
            case ExplainResult.Redirect redirect:
            ...
            case ExplainResult.Error error:
