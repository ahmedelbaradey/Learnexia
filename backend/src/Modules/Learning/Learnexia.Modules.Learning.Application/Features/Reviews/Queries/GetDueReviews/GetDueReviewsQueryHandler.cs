...
/// review for the JWT-authenticated student. Uses <see cref="ILearningRepository.GetDueMasteryRowsAsync"/>
...
/// UTC discipline: <c>DateTime.UtcNow</c> passed to the repository query.
/// </summary>
    ...
    : BaseResponseHandler, IQueryHandler<GetDueReviewsQuery, BaseResponse<IReadOnlyList<DueReviewDto>>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // The repository returns rows with Skill navigation populated (Include in GetDueMasteryRowsAsync).
            // We filter in-memory after the query to apply the StudentId IDOR guard cleanly.
            // For scale: if needed, add GetDueMasteryRowsForStudentAsync; deferred until needed.
            var allDueRows = await _repository.Learning.GetDueMasteryRowsAsync(utcNow, cancellationToken);
