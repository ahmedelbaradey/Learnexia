...
public class AddSkillCommandHandler : BaseResponseHandler, ICommandHandler<AddSkillCommand, BaseResponse<string>>
{
    ...
    public AddSkillCommandHandler(ILearningServiceManager service, ILoggerManager logger, IStringLocalizer<SharedResources> localizer)
    {
        _logger = logger;
        _service = service;
    ...
    public async Task<BaseResponse<string>> Handle(AddSkillCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await _service.SkillService.AddAsync<AddSkillCommand>(request, cancellationToken);
        }
        catch (Exception ex)
        {
