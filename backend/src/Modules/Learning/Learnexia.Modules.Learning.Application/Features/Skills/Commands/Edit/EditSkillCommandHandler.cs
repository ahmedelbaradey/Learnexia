...
public class EditSkillCommandHandler : BaseResponseHandler, ICommandHandler<EditSkillCommand, BaseResponse<string>>
{
    ...
    public EditSkillCommandHandler(ILearningServiceManager service, ILoggerManager logger, IStringLocalizer<SharedResources> localizer)
    {
        _logger = logger;
        _service = service;
    ...
    public async Task<BaseResponse<string>> Handle(EditSkillCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await _service.SkillService.UpdateAsync(request);
        }
        catch (Exception ex)
        {
