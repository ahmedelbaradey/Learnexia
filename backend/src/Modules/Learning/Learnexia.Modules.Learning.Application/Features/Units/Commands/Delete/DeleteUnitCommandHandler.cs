...
public class DeleteUnitCommandHandler : BaseResponseHandler, ICommandHandler<DeleteUnitCommand, BaseResponse<string>>
{
    ...
    public DeleteUnitCommandHandler(ILearningServiceManager service, ILoggerManager logger, IStringLocalizer<SharedResources> localizer)
    {
        _logger = logger;
        _service = service;
    ...
    public async Task<BaseResponse<string>> Handle(DeleteUnitCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await _service.UnitService.DeleteAsync(request.Id);
        }
        catch (Exception ex)
        {
