...
public class AddLessonCommandHandler : BaseResponseHandler, ICommandHandler<AddLessonCommand, BaseResponse<string>>
{
    ...
    public AddLessonCommandHandler(ILearningServiceManager service, ILoggerManager logger, IStringLocalizer<SharedResources> localizer)
    {
        _logger = logger;
        _service = service;
    ...
    public async Task<BaseResponse<string>> Handle(AddLessonCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await _service.LessonService.AddAsync<AddLessonCommand>(request, cancellationToken);
        }
        catch (Exception ex)
        {
