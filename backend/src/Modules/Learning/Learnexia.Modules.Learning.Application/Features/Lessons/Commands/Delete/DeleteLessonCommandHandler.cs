...
public class DeleteLessonCommandHandler : BaseResponseHandler, ICommandHandler<DeleteLessonCommand, BaseResponse<string>>
{
    ...
    public DeleteLessonCommandHandler(ILearningServiceManager service, ILoggerManager logger, IStringLocalizer<SharedResources> localizer)
    {
        _logger = logger;
        _service = service;
    ...
    public async Task<BaseResponse<string>> Handle(DeleteLessonCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await _service.LessonService.DeleteAsync(request.Id);
        }
        catch (Exception ex)
        {
