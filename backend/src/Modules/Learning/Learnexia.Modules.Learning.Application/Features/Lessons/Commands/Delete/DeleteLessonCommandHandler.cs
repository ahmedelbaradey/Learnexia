...
public class DeleteLessonCommandHandler : BaseResponseHandler, ICommandHandler<DeleteLessonCommand, BaseResponse<string>>
{
    ...
    public async Task<BaseResponse<string>> Handle(DeleteLessonCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var lesson = await _service.LessonService.GetLessonTrackedAsync(request.Id, cancellationToken);
            if (lesson is null)
            ...
            var contentBlocks = await _service.LessonService.GetContentBlocksTrackedAsync(request.Id, cancellationToken);

            foreach (var block in contentBlocks)
            {
                block.IsDeleted = true;
                await _service.LessonService.StageContentBlockUpdateAsync(block, cancellationToken);
            }
