...
- **Requirements:** FR-PAY-7 *(new — Phase 9)*
...
- **Cross-phase seam:** the `IGlobalSettingsProvider` *contract* + a **bootstrap-default implementation** can ship with the AI Helper (Phase 4) so the cache thresholds (P3-04/05/06) are coded against it from day one; **this story (P9-12) upgrades it to the DB-backed + Redis-cached + audited + admin-editable implementation** (mirrors the `ILearningContextProvider` Seeded→Rag pattern).
- **Consumed by:** P9-02 (grants), P9-03 (action costs), P9-04 (daily caps), P9-07 (pack price/size), P9-11 (admin write-surface), and P3-04/05/06 (cache thresholds). **P9-11 writes these via this provider** (supersedes the bespoke `BillingConfigVersion` mechanism).
...
- Admin-only edits, behind the P7/P9-11 admin console. Blocked by **P9-01** (Billing module / audit seam available).
