...
> **Family energy model (LOCKED 2026-06-16) — P10-13..17 wave:** energy is **parent/family-owned** (`FamilyEnergyAccount`) with **two non-convertible buckets** — (A) subscription/entitlement (temporary/monthly = `PlanEnergyPerSeat × ActivePaidSeats`, allocated per active-seat child, resets each cycle) and (B) purchased packs (permanent, shared family reserve). Per-child spend hits the child's **own allocation first**, then the shared purchased row as fallback. Seats define entitlement (only paid active seats mint energy); seat reduction triggers a **grace period** then enforcement (children never deleted, become NoSeat/Locked keeping all progress). Transfers are sibling-only; refunds apply to purchased energy only. **This supersedes P10-01's per-child `CreditAccount` ownership model** — a data migration is required (`credits.premium_monthly`/`credits.free_monthly` become `PlanEnergyPerSeat`). **Dependency order:** P10-13 (core) → P10-14 (seats) → P10-15 (lifecycle); P10-16 (redistribution) + P10-17 (refunds) build on P10-13.
