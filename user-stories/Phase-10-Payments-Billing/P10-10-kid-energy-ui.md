...
- **Requirements:** FR-CREDIT-5 *(new — Phase 10)*
...
## Notes
- **The only student-app billing surface — and it is read-only.** The child sees their energy and costs but **never** a payment/purchase/plan UI; the out-of-energy state routes to **"ask a parent"** (all top-ups happen in the parent app — specifically via **web checkout**, P10-05/06/07). The "ask a parent" CTA must not link to any native in-app purchase flow; it is informational only, routing to the parent's web checkout surface. Pipeline: `designer` → `frontend` → `frontend-e2e-tester`. Distinct from hearts (**P4-04**). Blocked by **P10-03**.
