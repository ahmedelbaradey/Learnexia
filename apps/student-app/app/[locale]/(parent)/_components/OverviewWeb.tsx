...
export function OverviewWeb({ onAddChild }: OverviewWebProps) {
  ...
  return (
    <Stack testID="overview-root" flexDirection="column" gap="$6" padding="$6" width="100%">
      {/* Header — plain row + document dir="rtl" flips once: title block lands on
           the logical-start side (right in AR, left in EN) and controls on the
           logical-end side. No row-reverse — that would double-flip in AR. */}
      <Stack testID="overview-header" flexDirection="row" alignItems="flex-start" justifyContent="space-between" gap="$4" flexWrap="wrap">
