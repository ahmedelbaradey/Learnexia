...
- **Batch 1 (foundation + avatar + register-consent) — done, PR open** on `feat/P1-12-FE`. Transport fix: `Google-SignIn`/`Forgot-Password`/`Reset-Password` added to `ANONYMOUS_PATH_SUFFIXES` (they're unauthenticated — must NOT carry the bearer). 5 api-client hooks created (`useUploadAvatar`/`useRemoveAvatar` consumed now; `useGoogleSignIn`/`useForgotPassword`/`useResetPassword` are foundation for batches 2-3). Avatar upload/remove in settings (web-only `<input type=file>`, PNG/JPG + 5 MB client cap, direct `$danger` remove; native picker deferred). RegisterForm now posts `acceptedTerms` (default false) + `country`. Typecheck 9/9 green; security-auditor PASS (0 blocking — anonymous-path change is not over-broad; avatar/ consent clean); reviewer PASS.
...
- **Next:** Batch 2 = forgot-password + reset-password screens + edit-child on web; Batch 3 = Google OAuth (expo-auth-session) + RTL/dark QA. Both unblocked (hooks already exist).
