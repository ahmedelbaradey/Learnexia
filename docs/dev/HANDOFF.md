...
- **P8-01-FE (add-child learning-language field) — done, PR open.** Branch `feat/P8-01-FE` off main. Required `learningLanguage` (ar/en) field on the add-child onboarding form (`AddChildForm`), fenced from the relabeled "App language" field (labelled group + helpers + ordering), auto-fills the child's UI language to match (guarded by `appLanguageTouched`, still editable), sent via `useAddChild` → `AddChildCommand.learningLanguage`. Design spec `design-system/ui_kits/onboarding/P8-01-FE.md`. Typecheck 9/9 green; security-auditor PASS (no Critical/High — backend validates the ar/en enum server-side); reviewer PASS.
- **Next:** P8-04-FE (parent change-learning-language + fresh-start warning) — needs a designer spec + a **blocking** security-auditor pass (destructive Math/Science progress reset + child data + `confirmFreshStart` gate).
