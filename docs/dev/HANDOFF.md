...
> 2026-06-12: **Phase 9 — Payment, Billing & Credits task breakdown — PLANNING ONLY, all 🔲 (same branch as the AI-phase breakdown).** Authored 11 user stories (`user-stories/Phase-9-Payments-Billing/P9-01..11`) + briefs + plans + task files (`tasks/Backend/Phase-9-Payments-Billing/`, `tasks/Frontend/student-app/Phase-9-Payments-Billing/` parent billing + child ⚡ energy meter, `tasks/Frontend/admin-dashboard/Phase-9-Payments-Billing/P9-11-FE` config). **The AI credit economy ("⚡ طاقة المساعد") + monetization** lifted out of the AI-Helper MVP (which keeps only a minimal daily-request cap). **Parent-driven: ALL purchasing/billing/payment is parent-side; the child only spends energy + sees a read-only meter (P9-10).** New **`Billing`** module owns the credit ledger (dual pool: monthly **granted**-expire vs **purchased**-persist), subscriptions, payments, config; spend reaches the AI Gateway via `Shared.Contracts` **`ICreditSpendService`** (charge-on-delivery, cache-hits charged same, no charge on refuse/error). **Lead decisions still open before build:** new `Billing` module sign-off, **payment provider Paymob vs Fawry** (P9-06/07), refund clawback policy (P9-09), EGP/VAT receipt fields (P9-08), Hangfire for grant/dunning jobs. **P9-03 (spend) is hard-blocked on the AI Helper cluster (P3-01..06) merging first.** Per-action costs: hint=1, explain-mistake=3, deep-explanation=5, practice-generation=5 (config-driven, P9-11). The AI-phase decisions doc (`docs/decisions-needed-ai-phase.md`) is now RESOLVED.
