...
- **🔴 HIGH but ⚠️ DEFERRED — P1-05 / `GradesController`** (`api/learning/Grades/*`): no `[Authorize]` at all — unauthenticated **Create/Update/Delete** of curriculum grades. **A fix attempt (gate writes `AdminOnly`, reads authenticated) was reverted** because it conflicts with an existing **P2-01 design decision**: `P2_01_CurriculumHierarchy_Tests` has **16 committed tests that assert the grade endpoints are anonymous *by design*** (e.g. `AC6_GradesList_IsAnonymous`, anonymous create round-trip). So this is **security posture vs. an existing contract** — a lead decision, not a silent fix. To gate it: also rewrite those 16 P2-01 tests + update the P2-01 story AC; to leave it: accept the anonymous-curriculum risk and keep the gap documented. **Currently UNGATED (status quo). Awaiting lead decision.**
