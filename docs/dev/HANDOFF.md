...
## Localization FE wave (in progress on `feat/P8-localization-FE`, 2026-06-06)
...
- **P8-99-FE (app-shell foundation) — done, PR open.** Brand-font runtime loading (cherry-picked from `feat/design-system-pixel-align` 328ef06) + web Arabic font stacks (`Poppins, Cairo`/`Poppins, Tajawal`); RTL fixes on parent web; persisted UI-language switch (`LanguagePanel` + `useUpdateUserLanguage` → `User.PreferredLanguage`, native-restart UX); i18n key sweep. Typecheck 7/7 green; lint clean for changed packages (17 pre-existing `packages/ui` lint errors from P2-05/06/07 remain — unrelated).
- **Next:** P8-01-FE (add-child learning-language field) then P8-04-FE (parent change-learning-language + fresh-start warning) — both now unblocked by the regen; each needs a designer spec + (P8-01 light / P8-04 blocking) security-auditor pass.
