...
- **Settings** screen pixel-perfect (`(parent)/settings.tsx`): six-tab rail via new `packages/ui` **`Tabs`** primitive; **Profile** (avatar/name/email/phone/country — Save + avatar are UI-first no-op stubs, `TODO(P1-12)`) + **Language & region** (functional — switches locale app-wide) functional; the other four tabs (Notifications/Linked/Security/Plan) are "coming soon" → **P2-12** (pending merge).
...
## What's next (web FE)
- **Landing** is the last P1-11 screen (Login, Register, My Children, Splash, Dashboard built; Settings built; Reports is a blank placeholder by product call). Landing lives in **`apps/marketing-site`** (Next.js — different stack).
