...
- **Batch 2 (password-reset screens + edit-child on web) — done, PR open** on `feat/P1-12-FE-batch2`. New `(auth)/forgot-password.tsx` (generic anti-enumeration success) + `(auth)/reset-password.tsx` (reads `?email=&token=`, status-only token-error classification, token never logged/echoed, success→login). Edit-child: `ChildDashboardCard` Edit affordance + `EditChildSheet` dual-mode (slim backend edit: fullName/grade/language/country → `useUpdateChild`). **Backend (lead-approved):** `LinkedChildResponse` gains `Grade`/`Language`(short ar/en)/`Country` via the `ChildProfile` seam so the edit form pre-fills REAL values (prevents a save silently overwriting them) — api-client regenerated. Typecheck 9/9 green; security-auditor PASS (0 blocking — edit-child IDOR closed server-side via `IsLinkedAsync`; reset token body-only + rate-limited; anti-enumeration generic at all layers); reviewer PASS.
- **Lead-resolved decisions:** Google **only** (Apple/MS = dimmed placeholders); Google client ID via **env** (`EXPO_PUBLIC_GOOGLE_CLIENT_ID`) — flow wired, not live-testable without an ID; reset deep-link uses `?email=&token=`; avatar web-only this story; edit-child uses a **slim edit-only field set** (no password re-entry) in the reused `EditChildSheet` shell; CAPTCHA out of scope (P1-11-FE-16).
- **Next:** Batch 3 = Google OAuth (expo-auth-session, client ID via env) + RTL/dark QA sweep. Unblocked (hooks exist).
