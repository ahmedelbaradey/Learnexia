## HOTFIX + FE E2E coverage hardening — 2026-07-01 (`test/fe-e2e-quiz-correct-path-and-auth-recovery`)

**Reviewer PASS. Contains a real shipped-bug hotfix uncovered by E2E coverage work.**

- **🔴 HOTFIX — `BackfillSkillKeys` migration crashed backend startup on any SEEDED DB (bug we shipped in BL-04).** `20260623012123_BackfillSkillKeys` derives `SkillKey` from node `Name`; multiple P2-11 `KnowledgeNodes` with the same slugified name in a subject+grade (e.g. nodes named "1","2"… → `math.grade1.1`) produced duplicate candidate keys, and the `NOT EXISTS` guard did NOT dedupe WITHIN the same UPDATE batch → `23505` unique-violation on `UX_KnowledgeNodes_SkillKey` → boot crash. **Fix:** added `COUNT(*) OVER (PARTITION BY candidate_key)` + `AND dup_count = 1` so within-batch duplicates are LEFT NULL (matches the migration's documented intent + BL-04 Q3 nullable decision). **Why it was latent:** BL-04's smoke test + all curriculum-pipeline integration tests ran against FRESH/EMPTY Testcontainers DBs, so the backfill updated 0 rows and never hit the duplicate path. **Regression test added:** `backend/tests/Curriculum.IntegrationTests/BL04_BackfillSkillKeys_Tests.cs` (2/2 green — the coverage gap that let it through is now closed). Editing the historical migration is correct: it crashes at `MigrateAsync()` before any new migration could run.
- **FE fix — quiz back-nav on deep-links (DEF-P205FE-02):** `apps/student-app/app/(child)/lessons/[lessonId].tsx` — `handleBack` + the intro-stage back now guard `router.back()` with `router.canGoBack()` and fall back to `/(child)/subjects/{id}` (or `/(child)/`) so a deep-linked quiz can navigate back with no in-app history. Mirrors the existing push-screen pattern (hearts/attempts/xp).
- **Verified, NOT bugs (were flagged, then disproven):** DEF-P205FE-01 (quiz grading) was already fixed in backend (`AnswerComparator`) — confirmed LIVE (`P2-06-FE` 34/34, correct answers grade `isCorrect=true` across all 4 question types). The forgot-password "enumeration defect" (RP-TC-08) was a MISDIAGNOSIS — the handler already returns an identical generic 200 for existing/non-existing/exception (`P6_06 AE2/AE3` integration tests pass 2/2); RP-TC-08's live failure was end-of-run harness exhaustion. **No backend auth change made.**
- **E2E coverage added/refreshed:** `P2-05/06/07-FE` + `carryover-d1` rewritten to assert the real correct-answer path (all 4 question types; DEF-P205FE-01 workaround removed); new `P1-forgot-reset-password-FE.spec.ts` (forgot/reset flows, anti-enumeration, RTL/ar+en, token-not-echoed). Corrected `docs/qc/fe-e2e-coverage-gaps.md` (DEF-P205FE-01 + "MCQ-only seed" were STALE audit items — both resolved in backend; all 4 question types ARE seeded by LearningSeeder Step 3).
- **Still-open FE defects logged by the coverage run (tracked, not fixed here):** `DEFECT-RTL-HTML-DIR` (app sets Tamagui internal direction, never `html[dir]` — a11y/tooling gap); missing `reports-*` testIDs in `ReportsWeb.tsx` (`reports-header`/`reports-range-select`/`reports-send-button`/`reports-attempts-panel`). See `docs/qc/fe-e2e-coverage-gaps.md`.

---

## BL-03 knowledge graph — FINAL Curriculum-pipeline story — 2026-06-24 (`feat/BL-03-knowledge-graph`)

**Status: COMPLETE — reviewer PASS, security PASS (0 Critical/High), api-tester 17/17 + Python 79/79 green.** 🎉 **This closes the Curriculum Intelligence pipeline: BL-04 ✅ → BL-01 ✅ → BL-02 ✅ → BL-05 ✅ → BL-03 ✅ (all merged except this PR).** Relational graph only (Neo4j deferred to Phase 3); reuses the P2-11 `KnowledgeNode`/`KnowledgeEdge` graph (no second graph).

**The flow (Decision E — suggestions NEVER auto-publish):** `BuildKnowledgeGraphSuggestionsCommand` (admin `POST /api/curriculum/kg-suggestions/build?subjectCode&gradeId`) enqueues a `PipelineJobs{JobType='infer_edges'}` row with the subject's KnowledgeNodes (by SkillKey) in `PayloadJson`. The Python `infer_edges` lane claims it, runs LightRAG/LLM edge inference (MOCKED/devops-gated), returns suggested edges as `ResultJson` (writes NO DB rows). The .NET `EdgeInferenceAdvanceService` (mirror of IngestJobAdvanceService) consumes Done/Failed jobs, resolves SkillKeys→node ids (fail-soft drop+log unresolved), re-clamps strength/confidence server-side, and writes `KGSuggestion{Pending}` — **NEVER `KnowledgeEdge`**. The ONLY path to `KnowledgeEdge` is admin approve.

**Two cross-process contracts (BOTH must stay in lockstep — a parallel-build INPUT divergence was caught+fixed here):**
- **INPUT** (`PayloadJson`, .NET `BuildKnowledgeGraphSuggestionsCommandHandler` emitter ↔ Python `inference/pipeline.py` `InferPayload.parse`): per-node `skill_key, title, node_type(STRING "Skill"/"Concept"/"Review"), subject_code(STRING lowercase "math"...), grade(int number), difficulty`. NOTE the int↔string subject/node-type maps live in curriculum as **frozen cross-process constants** (cannot reference learning enums — module isolation).
- **OUTPUT** (`ResultJson`, Python `inference/models.py` emitter ↔ .NET `EdgeInferenceAdvanceService` reader): `{inference_model, edges:[{source_skill_key, target_skill_key, relationship_type("Prerequisite"|"Related"), strength(0..1), confidence(0..1)}]}` (snake_case; relationship_type PascalCase).

**Two NEW Shared.Contracts seams (`Learnexia.Shared.Contracts.Learning`, impls in learning — mirror IPedagogicalTreeWriter; no project ref curriculum→learning):**
- `IKnowledgeEdgeWriter.PublishApprovedEdgeAsync(...) → EdgePublishResult(Published, Duplicate, Cycle, CrossLanguage, EdgeId?)` — the Decision-E publish chokepoint; encapsulates the cross-language + duplicate + acyclic (`SkillGraphValidator.AssertAcyclic`) guards INSIDE learning. The admin approve handler maps the result to 200/409(dup)/422(cycle|cross-language) in a single curriculum-side transaction; on reject/Cycle/CrossLanguage the suggestion stays Pending and no edge is written.
- `IKnowledgeNodeReader` (`GetNodesForSubjectAsync`, `GetNodesBySkillKeysAsync` → `KnowledgeNodeSummaryDto` incl. `GradeNumber` via a Grades join).

**Schema:** migration `20260624084010_MakePipelineJobDocumentIdNullable` — `PipelineJob.DocumentId` is now `int?` (FK optional). `infer_edges` is a graph-level job with NO source document (parse/ingest still set DocumentId). AlterColumn only, no re-create.

**Student-facing query gaps added** (learning `KnowledgeGraphController`, student `[Authorize]`): `GET /RelatedConcepts/{nodeId}` (related both directions, distinct) + `GET /RemediationPath/{nodeId}` (transitive upstream prerequisites for a weak skill; config `KnowledgeGraph:RemediationMaxDepth`=3 + cycle guard). KGSuggestion mgmt (list/approve/reject/build) is admin-only.

**Python:** new `inference/` subpackage + `workers/infer_poller.py` (third lane), `INFERER_BACKEND=mock` default; live `lightrag`/Claude only when `INFERER_BACKEND=lightrag` + `ANTHROPIC_API_KEY` set (degrades to mock with a warning). Prompt-injection hardened (system/user split, fenced `<curriculum_nodes>`, allowed-SkillKey whitelist). `main.py` runs all three pollers (parse/ingest/infer_edges).

**DEVOPS follow-up (gated, NOT blocking):** set `INFERER_BACKEND=lightrag` + `ANTHROPIC_API_KEY` + pin `lightrag-hku` (`[live]` extra) to flip live; mandatory prompt-injection re-audit at the flip. **Full step-by-step activation for all three gated stages (parse/Azure DI, ingest/Claude, infer/LightRAG) — env vars, compose edits, verification, rollback, and the per-stage mandatory security re-audit checklist — is in [CURRICULUM-PIPELINE-ACTIVATION-RUNBOOK.md](CURRICULUM-PIPELINE-ACTIVATION-RUNBOOK.md).** (Note the runbook flags a real gap: `EXTRACTOR_BACKEND` is not yet passed through in the compose `curriculum-intelligence` env block — must be added before the ingest live-flip.)

**Follow-up nits (non-blocking, logged by reviewer — pick up in a polish pass):** (1) `ListKGSuggestionsQuery` controller advertises `subjectCode`/`gradeId` params but the handler ignores them (status filter works; subject/grade filtering needs a cross-module join — drop the params or wire via IKnowledgeNodeReader). (2) `KnowledgeEdgeWriterAdapter` reports missing-node as `CrossLanguage=true` (misleading 422; fails closed — add a distinct `NodeMissing` flag). (3) `GetRemediationPathQueryHandler` N+1 node load (bounded by depth 3; batch `WHERE Id IN (...)` would be cleaner). (4) Platform-wide: no rate limiter on `POST /build` (or anywhere) — track with the global rate-limit follow-up.

**Carried-forward debt across the pipeline (for a future polish/coverage wave):** the `ContentSource (GradeId, FileName)` idempotency collision (BL-02/BL-05); provenance `PedagogicalNodeId` points at Skill not Concept (BL-05); the cross-module approve transaction is not a single distributed DB transaction (inherent to module isolation; guard-then-publish + duplicate-idempotent retry makes it safe — BL-05/BL-03); and the mandatory **live-flip re-audits** for Azure DI (BL-02 zip-bomb/XXE), Claude ingestion (BL-05 prompt-injection), and LightRAG inference (BL-03 prompt-injection) when the devops keys are provisioned.

---

**The lane:** a new `workers/infer_poller.py` (`InferPoller`) claims `PipelineJobs{JobType='infer_edges',Status='Pending'}` via the SAME atomic `claim_next(job_type=...)` (SKIP LOCKED + JobType filter — lane-isolated from parse/ingest). It reads the node hierarchy DIRECTLY from `PayloadJson` (no MinIO round-trip — .NET enqueue already shaped the nodes by SkillKey), runs the configured inferer, post-processes, and writes back the frozen ResultJson (Done) or diagnostics (Failed). New `inference/` subpackage mirrors `ingestion/`: `models.py` (contract), `inferer.py` (`EdgeInferer` protocol), `mock_inferer.py` (default), `lightrag_inferer.py` (live, gated), `factory.py`, `scoring.py`, `postprocessor.py`, `pipeline.py`. `main.py` now starts a THIRD daemon thread.

**FROZEN cross-process `ResultJson` contract (LOAD-BEARING — keep in lockstep with .NET `EdgeInferenceAdvanceService`):** defined by `inference/models.py` `to_dict`. Snake_case keys; `relationship_type` PascalCase string:
`{ "inference_model": "lightrag-mock-v1", "edges": [ { "source_skill_key": "<SkillKey>", "target_skill_key": "<SkillKey>", "relationship_type": "Prerequisite"|"Related", "strength": <float 0..1>, "confidence": <float 0..1> } ] }`. SkillKeys use the SAME `ingestion/skill_key.py` derivation (`{subjectCode}.grade{N}.{unitSlug}.{leafSlug}`) — .NET resolves SkillKey→node id and DROPS unresolved keys. The .NET side RE-CLAMPS strength+confidence (advisory only). A terminal failure writes `{"infer_status":"failed","diagnostics":{...}}` and NEVER touches RetryCount (.NET owns retry). Contract proven by `tests/test_infer_contract.py` (mirror of `test_ingest_contract.py`).

**Mock-vs-live (Q3, devops-gated like BL-02 Azure DI / BL-05 Claude):** `INFERER_BACKEND=mock` (DEFAULT, dev+CI) → deterministic `MockEdgeInferer` (difficulty-ordered prereq chain per subject+grade + Related sibling edges; no network). `INFERER_BACKEND=lightrag` + `ANTHROPIC_API_KEY` → `LightRagEdgeInferer` (LLM-driven; LightRAG graph-store `lightrag-hku` pinned at provisioning, follow-up). Missing key → degrades to mock with a warning (CI can never call out). Prompt-injection hardening copied from `claude_extractor.py`: schema/instructions in SYSTEM prompt, untrusted node text fenced in `<curriculum_nodes>` USER block with `<`/`>` escaping; the mapper drops any edge whose SkillKey isn't in the supplied set or whose relationship isn't `Prerequisite`/`Related`.

**New env keys (`.env.example` + compose):** `INFERER_BACKEND` (default mock), `INFERENCE_MODEL` (default `lightrag-mock-v1`), `INFER_ANTHROPIC_MODEL`, `INFER_MIN_CONFIDENCE` (advisory floor, default 0 = keep all), `INFER_POLL_INTERVAL_SECONDS`/`INFER_POLL_JOB_TYPE`/`INFER_POLL_BATCH_SIZE`. Compose adds `INFERER_BACKEND=${INFERER_BACKEND:-mock}` to the `curriculum-intelligence` service; reuses the existing `ANTHROPIC_API_KEY` `${VAR}` ref (never committed). `pyproject.toml` adds the `inference` package; the `[live]` extra's `anthropic` covers the LLM driver.

**DEVOPS follow-up (gated, NOT blocking the build):** set `INFERER_BACKEND=lightrag` + `ANTHROPIC_API_KEY` (+ pin `lightrag-hku` and wire the graph store) to flip live; re-audit prompt-injection defense-in-depth at the flip (cap injected node count, fence nonce) as for BL-05.

---

## BL-05 curriculum ingestion — LLM structuring + cross-module tree writes — 2026-06-24 (`feat/BL-05-curriculum-ingestion`)

**Status: COMPLETE — reviewer PASS, security PASS (2 blocking Highs fixed + re-audited), api-tester 28/28 + Python 57/57 green.** Fourth story of the Curriculum Intelligence pipeline (BL-04 ✅ → BL-01 ✅ → BL-02 ✅ → **BL-05** ✅ → BL-03). Same Python service + DB-outbox pattern as BL-02 (ADR-0004), new `ingest` lane.

**The flow:** a successful PARSE job now enqueues a `PipelineJobs{JobType='ingest',Status='Pending'}` row (hand-off added to `ParseJobAdvanceService` Done path, Q8). The Python ingest lane claims it, reads the parse artifact from MinIO, runs LLM hierarchy extraction + semantic chunking + confidence scoring, and returns a structured `ResultJson` (writes NO DB rows — Q6). The .NET `IngestJobAdvanceService` consumes Done/Failed ingest jobs and owns ALL DB writes.

**Cross-process `ResultJson` contract (LOAD-BEARING — both sides MUST stay in lockstep):** defined by Python `python/curriculum_intelligence/ingestion/models.py` `to_dict`, read by .NET `IngestJobAdvanceService` ParseNode/Chunk/FlagArray. Snake_case; node_type PascalCase. node: `node_type, skill_key, parent_skill_key, title, grade(int), subject_code(STRING "math"...), language(STRING "ar"/"en"), difficulty(int?), confidence(float)`; chunk: `content, content_type, source_page(int?), chapter_number(int?), node_skill_key, confidence`; flag: `kind, ref, reason, confidence`. **A parallel-build divergence here caused 7 field mismatches caught by api-tester+security — fixed by aligning .NET to Python. Any future change must update BOTH sides + the contract test `CONTRACT_PythonShape_AgreementAfterFix`.**

**Cross-module write seam (Q1 — lead-approved):** `IPedagogicalTreeWriter` in `Shared.Contracts/Learning/`, implemented by `PedagogicalTreeWriterAdapter` in the LEARNING module. Curriculum calls it SYNCHRONOUSLY (request/response, returns node ids) to create Unit/Lesson/Concept/Skill — NOT a MediatR INotification (those swallow exceptions + return nothing). Idempotent on `SkillKey` (ownership stays in learning). **No project ref curriculum→learning; Shared.Contracts only; learning node refs from curriculum are plain ints.** The Python `skill_key` derivation (`ingestion/skill_key.py`) matches the .NET BackfillSkillKeys format `{subjectCode}.grade{N}.{unitSlug}.{leafSlug}` so re-ingest is idempotent on both sides.

**Decisions baked in:** Q4 **embeddings DEFERRED** (ingest writes CurriculumChunk rows only; vector generation stays with P3-07's re-embed path; no inline vector, Decision D) · Q5 low-confidence (< `CurriculumPipeline:IngestionConfidenceThreshold`, default **0.7**, config-driven) → `IngestionReviewItem{Pending}` (NEW curriculum table, separate from KGSuggestion), withheld from the tree; admin approves/rejects via `IngestionReviewItemsController` (approve promotes into the learning tree in a single transaction) · **confidence is FAIL-CLOSED** (missing/out-of-range → 0.0 → review; server-side clamp — do not trust the LLM's raw value) · Q7 ingest targets a **DRAFT** CurriculumVersion only (never auto-Active; P7-05 publishes) · Q3 Claude **mocked-now/devops-gated** (`EXTRACTOR_BACKEND=mock` default; `claude` only when `ANTHROPIC_API_KEY` set; `ANTHROPIC_MODEL` default `claude-sonnet-4-6`). New `IngestionStatus` axis (`NotStarted/InProgress/Done/Failed` + `IngestedAt`/`IngestionDiagnostics`) on CurriculumDocument is distinct from the parse `DocumentStatus`. Migration `20260623220520_AddIngestionReviewItems`.

**Security hardening applied (audit `docs/security/BL-05-audit.md`):** untrusted `ResultJson` bounded to column maxes + no-stranding terminal write (BL-02 pattern); all `IngestionReviewItem.PayloadJson` paths serialize valid jsonb (fixed a bare-string `22P02` crash); LLM prompt-injection mitigated — Claude prompt puts schema in the SYSTEM message and untrusted document text in a fenced `<document_text>` USER message with collision-escaping (`ingestion/claude_extractor.py`).

**DEVOPS follow-up (gated, NOT blocking the merged build):** set `EXTRACTOR_BACKEND=claude` + `ANTHROPIC_API_KEY` (+ confirm model tier/cost ceiling for full-textbook ingestion) to flip live. **Mandatory live re-audit** at the flip for the deferred prompt-injection defense-in-depth (cap injected node/chunk count, randomized fence nonce, truncation behavior).

**Notes for BL-03 (next):** (1) ingestion provenance `ProvenanceMapping.PedagogicalNodeId` points at the **Skill** (finer-grained anchor), NOT the Concept — `PedagogicalNodeType="Skill"`; SkillKey retained for correlation. (2) Carried Q9 nit: `ContentSource` idempotency `(GradeId, FileName)` can collide two different docs sharing grade+filename (TODO in `UpsertProvenanceMappingAsync`). BL-03 (knowledge graph) consumes the structured tree + feeds the `KGSuggestion` review queue.
...
> **⚠️ PROD DEPLOY CHECKLIST (devops):** the live payment provider is still **NOT implemented** — `FakePaymentProvider` is the dev/staging mock. Before prod: (1) set `Billing:PaymentProvider:Provider="Paymob"`, (2) keep `Billing:PaymentProvider:AllowSimulation=false`, (3) implement the real Paymob API calls inside `PaymobPaymentProvider` (P10-06-BE-3 — blocked on the provider decision + sandbox keys/webhook secret). The default `Provider` is `Fake`, so a prod deploy that forgets (1) keeps the mock active.

> **Update 2026-06-23 — real-adapter slot wired with a STUB.** `PaymobPaymentProvider` now fills the `Provider="Paymob"` branch (`DependencyInjection.cs`) instead of silently falling back to `FakePaymentProvider`. It boots without secrets but **throws "not configured" on every payment op** (incl. `VerifyWebhookSignature` — never admits an unverified webhook), so a non-Fake config can NEVER mock-approve a payment. Tests: `P10_PaymobStub_Tests` 2/2; reviewer PASS (also confirmed the simulate-webhook path is gated off under non-Fake). Going live = fill in the Paymob API calls + keys; no other code change.
...
**Root cause found + fixed (load-bearing):** the JWT bearer pipeline had **no `OnTokenValidated`** (tokens accepted on signature + lifetime only), AND the session store was wired wrong — `AuthenticationIdentityService.GetClaims` minted the `"SessionId"` claim as one GUID while `SessionManagementService.CreateSessionAsync` keyed the Redis session on a *different* server-generated GUID. So every `TerminateSessionAsync(GetClaimValue("SessionId"))` in sign-out / sign-out-others / change-password was **silently missing** — the whole P1-02/P2-12/P7-07 session-termination machinery was effectively a no-op. Both are fixed here.

**What shipped (Identity + Host only; no new module; NO DB migration — sessions are Redis/`IDistributedCache`; no UoW; no new NuGet):**
- **Mismatch fix (BE-1):** `CreateSessionAsync(userId, jwtTokenId, sessionId)` is now 3-arg and **keys the session on the JWT's `"SessionId"` claim**. All **four** token-issuance paths thread the claim through: `SignInCommandHandler`, `RegisterParentCommandHandler`, `GoogleSignInCommandHandler`, **and `RefreshTokenCommandHandler`** (the 4th was necessary — `GetRefreshToken` mints a *new* SessionId claim on rotation, so without a matching session every refreshed token would 401). The stale `catch { /* middleware creates it */ }` comment (no such middleware exists) replaced with a loud error log in each.
- **Per-request validation (BE-2):** `JwtBearerEvents.OnTokenValidated` in `Identity.Infrastructure/DependencyInjection.cs` `AddJwtBearer` reads the `"SessionId"` claim and validates it **read-only** (single Redis GET, **zero writes** — does NOT call `ValidateSessionAsync(...,true)`, which would write sliding-expiration every request). Absent/terminated/expired session → `context.Fail("session_revoked")` → 401. Scoped `ISessionManagementService` resolved from `context.HttpContext.RequestServices` (NOT captured in the singleton options closure).
- **Fail-mode (BE-4 / OQ-1):** **fail-closed in Production/Staging, fail-open + `LogWarn` in Development/Testing**, via the existing `IsProtectedEnvironment(configuration)` helper. Implemented explicitly with a new **non-swallowing** `ISessionManagementService.IsSessionActiveAsync` probe: a genuine revocation (absent/inactive/expired) deterministically returns `false` → fail-closed **in every env** (a revocation can never fail-open); only a real store/deserialize **fault** propagates → the env-gated catch. (Plain `GetSessionAsync` still swallows for its other callers; the dead-code `GenerateSessionId()` was removed.)
- **Revoke-all (BE-3):** `ResetPasswordCommandHandler` (anonymous — no current session to keep) now terminates **all** the user's sessions via the new `ISessionManagementService.TerminateAllUserSessionsAsync(userId, reason)`. `SuspendAccountCommandHandler` + `AccountDeletedDomainEventHandler` already terminated all — refactored onto the shared helper (DRY).

**Locked decisions (`docs/plans/P6-07.md`):** OQ-1 fail-mode (above) · OQ-2 read-only hot path (drop per-request sliding-expiration refresh; token≈session TTL both 30m) · OQ-3 keep the issuance try/catch but log loudly (don't hard-fail sign-in) · **OQ-4 ⚠ one-time logout-all on deploy is ACCEPTED** — existing pre-deploy access tokens carry the old mismatched SessionId with no matching session, so every currently-signed-in user is forced to re-auth once. Pre-launch; and it's the desired effect of the fix.

**Gates:** build 0 errors · api-tester `P6_07_TokenRevocation_Tests` **13/13** + `P1_09_Me_Tests` **28/28** (incl. the flipped `BE-TC-17 Leg 2`, which was *written* to document the old 200 "JWT-not-revoked" behavior and now correctly asserts **401**) + regression P1_02 26/26, P2_12 39/39, P1_12_BE6 28/28, P7_06_07_08 90/90 · Identity unit 14/14 · **security-auditor PASS** (no Critical/High/Medium; its one Info finding — dead dev fail-open branch — was fixed via `IsSessionActiveAsync`) · reviewer PASS.

**Deferred non-blocking nits (NOT in this PR):** `RefreshTokenCommandHandler` `int.Parse(userId)`→`TryParse` (cannot crash — validated JWT + prior `FindByIdAsync` success); `ExtractSessionInfoFromToken` is duplicated as a local helper across the issuance handlers (a future DRY pass — left local per CONVENTIONS rule #8: ask before adding a shared abstraction). **FE follow-on:** `tasks/Frontend/Phase-6-Stabilization/P6-07-FE.md` (handle a mid-session 401 → single refresh-retry → redirect-to-login; owned by the FE lead, not built here).
...
**✅ DEF-GAM-01 (Medium) — FIXED 2026-06-20 (`fix/DEF-GAM-01-expire-timed-event`).** Added a domain `TimedEvent.Expire(nowUtc)` (sets `IsActive=false` AND rewinds `EndUtc = nowUtc`) and pointed `GamificationAdminService.ExpireTimedEventAsync` at it (replacing the bare `Deactivate()`, which is kept for the `TimedEventSweepJob` natural-window-exit path — that must NOT rewind `EndUtc`). So a manually-expired event now renders ENDED (not SCHEDULED) in the timestamp-based FE. **Gotcha hit + fixed:** the `CK_TimedEvents_DateRange` CHECK (`StartUtc < EndUtc`, strict) tripped because the read-back `StartUtc` is machine-LOCAL Kind under `EnableLegacyTimestampBehavior` → the start-clamp compared local-vs-UTC wrong and set `StartUtc == EndUtc`; fixed by comparing `StartUtc.ToUniversalTime()` and clamping a genuinely-future start to `EndUtc.AddSeconds(-1)`. (Note: the Gamification `UnitOfWorkBehavior` wraps ANY `DbUpdateException` as `GamificationUniqueConstraintException` "Unique constraint violation…", so that message can actually mask a CHECK-constraint violation.) Regression test: `P7_13_GamificationAdmin_Tests.ExpireTimedEvent_RewindsEndUtc` (+ 12/12 TimedEvent tests green). The FE GAM-TC-23 soft-assert can become a hard-assert now.
...
**What shipped:**
- **Data model:** `TimedEventParticipation` (`Create` + `ApplyProgress` clamp/single-completion-latch), `TimedEventParticipationStatus {NotStarted,InProgress,Completed}` (deliberately NOT `MissionStatus` — no Expired; the row freezes in place, the scan filters by `EventEndUtc`), `TimedEvent.ParticipationTarget int?` (nullable → no backfill; null falls back to config `DefaultTarget`), migration `P4_12_AddTimedEventParticipation` (table + column + CHECK constraints + unique `(TimedEventId, StudentXpProfileId)` index).
- **Accrual:** hooked into `XpService` BOTH award methods, AFTER the XP idempotency guard (no double-count). Lazy-create on first qualifying **AllXp**-scope in-window action (Mission/League scope = forward-compat no-op, as the boost calc already leaves them); `ApplyProgress(+1)`; completion grants reward via the EXISTING engine (`StudentXpProfile.RecordTimedEventCompleted`→`ApplyAward`, new `XpReason.TimedEventCompleted=7`) — no parallel path. **Stage-only** (Gamification's own `UnitOfWorkBehavior` commits + dispatches domain events post-commit — do NOT add SaveChangesAsync).
- **Events/seams** (`Shared.Contracts/Gamification`, opaque ids only): `TimedEventParticipation{Progress,Completed,EndingSoon}IntegrationEvent` via 2 republishers (progress=halfway one-time, completed) + the scan (ending-soon). `IEligibleStudentsForTimedEventQuery` (recruitment cohort = in-module `StudentXpProfile.LastActivityDateUtc` within `EligibilityWindowDays`, `Take(500)` — NO cross-module activity seam) + `IStudentTimedEventParticipationQuery`.
- **Scan:** `StreakAtRiskJob` **Pass 4** — in-progress participations with `EventEndUtc` within `EndingSoonLookaheadHours`, one event/student (most-urgent), fail-soft per student.
- **Endpoint:** `GET /api/Gamification/TimedEventParticipations/Me` (JWT/IDOR-safe — studentId from `ICurrentUserService`), localized `SharedResourcesKey.TimedEventParticipationsRetrievedSuccessfully` (EN+AR). Config `TimedEventParticipationOptions` (`Gamification:TimedEventParticipation`: DefaultTarget/HalfwayThresholdPercent/EndingSoonLookaheadHours/EligibilityWindowDays).

**🐛 Real UTC bug found + fixed (reusable lesson):** the ending-soon scan computed `MinutesRemaining = (row.EventEndUtc - nowUtc)` — but under **`EnableLegacyTimestampBehavior=true` a `timestamptz` reads back as machine-LOCAL Kind**, so on a non-UTC host (this dev box = +03:00) `MinutesRemaining` was off by the host offset. (Server-side WHERE filters were always correct — only the C# arithmetic on the read-back value was wrong.) Fixed with `.ToUniversalTime()` before the subtraction. **The SAME bug existed in P9-06's Pass 3 (weekly mission reminder, already merged) — fixed here as a drive-by.** **Rule of thumb: any C# arithmetic/comparison on an EF-read `timestamptz` must `.ToUniversalTime()` first** (filters that run server-side are fine). The api-tester's EndingSoon tests were also initially weak (built a fake event) — rewritten to capture the REAL published event + assert `MinutesRemaining` within tolerance, which now guards this.

**Gates:** build 0 errors; api-tester 22/22 (`P4_12_TimedEventParticipation_Tests` — accrual/no-double-count/completion-reward/halfway/eligibility/IDOR/enum-sync + real EndingSoon capture); security-auditor PASS (2 Info: cohort >500 determinism, test-only SQLite advisory); reviewer PASS. Reviewer cleanups applied: removed 3 unused repo methods + 2 projection records; wired the localized success message.
...
> 2026-06-18: **Admin Console Batch-1 QC + E2E DONE — admin E2E harness now EXISTS (load-bearing).** `qc-test-designer` wrote 77 cases to `docs/qc/P7-admin-batch1/`; `frontend-e2e-tester` implemented `tests/e2e/specs/P7-admin-batch1.spec.ts` and ran it **69 passed / 8 skipped / 0 failed** (no real app bugs; no missing testIDs). **How admin E2E works (reuse for future admin waves):** (1) new Playwright **`admin` project** in `tests/e2e/playwright.config.ts` (baseURL `http://localhost:3001`, `testMatch **/specs/P7-admin-*.spec.ts` / `admin-*`, own `webServer` = `pnpm --filter @learnexia/admin-dashboard dev` with **`NEXT_PUBLIC_API_URL=http://localhost:5080` set in the webServer env** — the admin app THROWS without it; there is no `.env.local`). Run: `cd tests/e2e && npx playwright test --project=admin --workers=1`. (2) **Admin login = the dev SuperAdmin seed**: `/login` form posts `{userName,password}` → `POST /api/Users/Authentication/Sign-In`; creds **userName `superadmin` / password `123Pa$$word!`** (seeded only when `ASPNETCORE_ENVIRONMENT=Development`; roles Admin+SuperAdmin satisfy `AdminOnly`). NOTE the userName is `superadmin`, NOT the email. (3) **CORS:** backend `AllowedOrigins` now includes `http://localhost:3001` (appsettings.json) — required for the admin app's browser calls; restart the Host to pick it up (config is read from the Host content-root at startup, no rebuild). (4) Test data seeded via API (`Register-Parent` + `api/Parent/Add-Child` — Add-Child no longer needs country). **2 shipped copy bugs fixed** in `users/[id]/edit/page.tsx` (profile-save was showing grade-success copy; 422 mapped to grade-range error) + testIDs added across all admin screens. **KNOWN LIMITATION:** admin app locale is a **build-time constant `ADMIN_LOCALE='en'`** — runtime ar/RTL is unreachable, so the 3 RTL cases are BLOCKED (ar strings verified present in `lib/strings.ts`); a runtime locale toggle is a separate story. 3 pagination + 2 row-click cases skipped (need >20 seeded users / target row not on page 1). On branch `test/P7-admin-batch1-qc-e2e`.
...
> 2026-06-18: **P10-18 (Pause Child Access) — BUILT + gated GREEN. FINAL wave story.** On `feat/P10-18-pause-child` (stacked on #168). **Delivered (BE-1..8):** `ParentPauseState{Active,Paused}` + `ParentPauseStateChangedAt` on `SeatReservation` (migration `20260618005545_AddParentPauseStateToChildSeat`, w/ Designer); `IPauseChildService` (pause/unpause — family-scope IDOR guard, idempotent same-state no-op, audit-ledgered to `SeatLedgerEntry{ParentPause/ParentUnpause}`, **ZERO energy/seat writes**); independent pause gate in `CreditSpendService` (`DebitOutcome.ParentPaused`); `IChildAccessStateQuery` (`IsChildAccessAllowedAsync` + `GetAccessDecisionAsync→{Allowed,SeatLocked,Paused}`) Shared.Contracts seam; CQRS + `ChildAccessController` (`POST Pause`/`Unpause`, `GET AccessStatus`, parent-only); 8 localized keys. **KEY FIX (security FAIL→PASS):** the pause gate was debit-only → a **paused child still got AI** (pause doesn't remove the allocation, so pre-flight balance>0 → delivery; the debit-time gate fired post-delivery + cache-HIT discarded it). Fixed by wiring the access gate into **all 4 AI handlers' pre-flight** (before cache+LLM): paused→`ChildAccessPausedByParent`, seat-locked→`ChildSeatLockedNoEnergy` (**this also closes the deferred P10-15 locked-child-message gap**), fail-soft on exception, + post-debit defense-in-depth. **Reviewer should-fix FIXED:** the pause ledger `IdempotencyKey` was day-scoped → a same-day pause→unpause→re-pause collided on the unique index → 500; changed to **per-occurrence** (`{type}:{p}:{c}:{ticks}`; the same-state no-op guard already dedups genuine double-submits) + PAUSE-08 regression test + `OrderByDescending(ReservedAt)` parity nit. **Gates:** build 0; Ai unit **297/297** (incl 10 new AG access-gate tests); Billing unit **147/147**; blast-radius **254/254**; api-tester **11/11** (`P10_18_PauseChild` incl PAUSE-06 paused-child-denied-AI-pre-LLM + PAUSE-08 same-day-repause); security-auditor PASS; reviewer PASS. No money. **DEFERRED follow-up:** none new (2 Info notes addressed). The P10-15 locked-child-AI-message deferred item is now RESOLVED by this story's gate.
> 2026-06-18: **P10-17 (Refund reconciliation — unused purchased energy) — BUILT + gated GREEN.** Batch-4 story 2 of 3, on `feat/P10-17-refunds` (stacked on #164). **Delivered (BE-1..6):** `IRefundService.ComputeRefundableAsync` (refundable = monetary value of UNUSED purchased energy = purchasedTotal − consumedPurchased − alreadyRefunded, **per-original-`Purchase`-payment FIFO**, **bucket-B/shared-purchased ONLY — bucket A subscription/granted NEVER refundable**, clamped ≥0, capped at current shared balance) + parent refund request/quote (`[Authorize(Roles=Parent)]`, family-scope) + **admin** refund request/quote (`[Authorize(Policy=AdminOnly)]`, any family, actor-id ledgered — OQ-2 = parent+admin) + the `refund.succeeded` webhook settlement (decrements shared `PurchasedBalance`, idempotent, clamped no-negative) + `RefundReason` enum + localized keys. **REFINES the P10-09 RefundService/webhook** (no parallel path). **No migration** (reused `CreditTransaction` cols + `PurchasedRefund` reason). **Security-auditor FAIL→PASS (caught 2 real money bugs):** **(CRITICAL)** distinct-event-id OVER-REFUND — the pack refund was idempotent only per-event-id, no `Status==Refunded` guard, and the reconcile didn't subtract prior refunds → a 2nd `refund.succeeded` (distinct event id) after a balance-replenishing repurchase paid out AGAIN → FIXED via per-payment idempotency key `pack-refund:{paymentId}` + `Status==Refunded` guard + subtract-already-refunded in the reconcile (3 layers). **(HIGH)** wallet-decrement lost-update race → FIXED via xmin optimistic-concurrency + bounded retry (HARDEN-01 pattern). **(Low)** explicit `payment.ParentUserId==wallet.ParentUserId` IDOR assertion. **Gates:** build 0; Billing unit **134/134**; api-tester **18/18** (incl new TC-RF-S1 distinct-event-id no-double-refund, TC-RF-S2 repurchase-in-between, TC-RF-S3 full-refund-re-request, TC-RF-S4 IDOR); security re-check PASS; reviewer PASS. **KNOWN-RED note discovered here: 3 daily-cap tests (`ENERGY-6A/6B`, `BE-TC-15`) are PRE-EXISTING TIME-OF-DAY FLAKE** — the tests seed `DailyUsedDateLocal = UtcNow` date but the cap-reset (`DailyCapHelper.IsStale`) uses the child's **Cairo (UTC+3) local** date, so near the UTC↔Cairo day boundary the seeded `DailyUsed` resets → cap not reached. Unchanged by any P10 batch; fails on main too at the same UTC window. **Follow-up: fix the test seeding to use the child's local date.** (Verified NOT the seat gate — the spend went through.) **Also surfaced: the P3_AI build break (CreditAccount→wallet) → hotfix PR #166 to main.** Open PRs: #162, #164, #166, + P10-17.
> 2026-06-18: **P10-16 (Family Energy Redistribution) — BUILT + gated GREEN.** Batch-4 story 1 of 3, on `feat/P10-16-redistribution` (off `feat/P10-15-seat-lifecycle`, stacked on #162). **Delivered (BE-1..7):** `IFamilyAllocationService` (`GetFamilyAllocationAsync` returns ALL active-seat children incl. mid-cycle ones with `IsMidCycleNoGrant`; `TransferAllocationAsync` moves UNSPENT allocation between the parent's OWN children) + CQRS query/command/validator + `FamilyEnergyController` routes + 6 localized keys. **Invariants (gates verified):** same-family IDOR guard (both child ids must be the JWT parent's; controller now `[Authorize(Roles="Parent")]` — the AUTH-01 fix, child JWT→403, also hardens the P10-13 Overview endpoint); only-unspent movable (`amount ≤ source.Remaining`); **bucket A only** (never touches shared `PurchasedBalance`, never converts, zero-sum on Remaining); mid-cycle-added child → INSERT allocation row then transfer; paired `TransferOut`/`TransferIn` ledger rows on `CreditTransaction` (energy ledger) with shared `CorrelationId`; idempotent on key; NoSeat/Locked child rejected as source/dest. **No migration** (reused P10-13's `CorrelationId`/`IdempotencyKey`/`TransferIn`/`TransferOut`). **Gates:** build 0; Billing unit **119/119**; blast-radius **217/217**; api-tester **19/19** (`P10_16_FamilyAllocation`); security-auditor **PASS (0 Crit/High, 6 invariants hold)**; reviewer **PASS**. **DEFERRED P10-16 follow-ups (non-blocking):** (1) **transfer-tx concurrency retry** — concurrent same-source transfers are CORRECT (xmin prevents over-draw) but the loser returns HTTP 500 instead of a clean 409/retry; add the `CreditSpendService` `DbUpdateConcurrencyException` retry pattern to `FamilyAllocationService.TransferAllocationAsync`. (2) **transfer reporting semantics** — `ChildEnergyAllocation.SendTransfer` increments `SpentAmount` instead of decrementing `AllocatedAmount`, so per-child Allocated/Spent shown to the parent inflate after a transfer (Remaining stays correct; no charging/security impact) — model transfer-out as an Allocated decrement or a dedicated `TransferredOut` column. (3) idempotency key not payload-bound (fails closed); (4) `long amount`→`int` cast unreachable-but-unguarded (add `≤ int.MaxValue` validator bound); (5) no per-endpoint rate cap on Transfer.
> 2026-06-17: **P10-15 (Seat Lifecycle Correctness) — BUILT + gated GREEN, ready to commit.** Merge-order gotcha RESOLVED: P10-14 landed in `main` via **PR #161** (`feat/P10-13-family-wallet` → main; the local union-merge was clean — GitHub's "not mergeable" was a stale check). `main` now contains P10-13+P10-14. **P10-15** built on `feat/P10-15-seat-lifecycle` (off `feat/P10-14-child-seats`; shares `7c3ca4f` with main → clean PR base = main). **Delivered (BE-1..11):** `SeatState{Active,NoSeatLocked}` + grace fields; `SeatGraceService` (payment-failure-only, idempotent no-stacking, `seats.grace_days=7`); `SeatSchedulingService` (cycle-end voluntary removal); `SeatEnforcementService` + Hangfire job (over-limit→NoSeat/Locked, earliest-reserved tie-break, removes future allocation row but **NO energy forfeit**, never deletes child, **applies the P10-14 `PendingExtraSeatRemovals` cancel marker at renewal**); seat-based spend gate (`DebitOutcome.SeatLocked` in `CreditSpendService`, denies before any balance touch); parent CQRS (`GetSeatStatus`/`ChooseActiveChildren`/`ReactivateChildSeat`, family-scoped IDOR-guarded); reactivation webhook branch (prorated money, **zero energy mint**); `ISeatStateQuery` Shared.Contracts seam; `SeatController`. **B (#3 race) FIXED:** per-parent `SELECT…FOR UPDATE` (Npgsql) + per-attempt placeholder (negative `ChildId` from key hash) + activate/release **by reservation key** + a per-attempt nonce in `AddChildCommandHandler`'s key (the email-derived key caused a dup-email 500 regression — fixed). **C (audit ledger) FIXED:** new `SeatLedgerEntry` (energy `CreditTransaction` untouched); all seat events (purchase/cancel/lock/reactivate/choice/removed) ledger to it with deterministic idempotency keys. Migration `20260617155237_AddSeatGraceAndLockState` (regenerated WITH Designer/[Migration] by db-migration — the implementer's hand-written one lacked it). **Gates:** build 0 err; Billing unit **109/109**; integration blast-radius (P10_15+P10_14+P10_13+P10_01_12+P1_03+P2_12) **217/217**; api-tester P10_15 **22→26 cases**; security-auditor **PASS (0 Crit/High)**; reviewer **PASS**. TC-GS-04 bumped 21→22 (`seats.grace_days` seeded). **DEFERRED P10-15 follow-ups (non-blocking, reviewer/security should-fix):** (1) **locked-child AI message** — a NoSeat/Locked child is correctly BLOCKED (allocation removed → `GetBalanceAsync` TotalBalance=0 → AI pre-flight denies; verified NO leak), but sees the generic `AiInsufficientEnergy` copy, not `ChildSeatLockedNoEnergy`; add a seat-state pre-flight (`ISeatStateQuery`) to the 4 AI handlers for the friendly message. (2) **stale-Reserved-placeholder sweep** — if reserve commits then BOTH child-create AND the compensating release fail, the negative placeholder stays `Reserved` and counts against the seat limit forever (enforcement excludes `ChildId<=0`); add a sweep of old `Reserved` placeholders to `SeatEnforcementJob` (narrow double-failure edge, recoverable, no data/security impact). (3) **grace wiring dedup** — the P10-09 dunning path sets grace inline in `RefundService` rather than via `ISeatGraceService`; route through the service for one source of truth. **STRAY (not P10-15):** `P3_AI_RuntimeActivation_E2E_Tests.cs` is modified in the tree (a P10-13 wallet-migration test-infra leftover, `SeedBalanceAsync` rewrite) — EXCLUDED from the P10-15 commit; resolve separately.
...
> 2026-06-17: **Parent-app E2E reconcile + guard fix + Playwright unblock (`feat/parent-e2e-reconcile`, off main).** Closes the gap the Batch-A parent-app flow opened. **(1) Real app bug fixed:** `useGroupGuard` still routed signed-out→`/(auth)/login` and parents-with-children→`/(parent)/overview`, inconsistent with `useAuthRoute` after the "land on My Children" change. Fixed → `/(auth)/role-select` and `/(parent)/children`. (Overview is the per-child view now, reached by tapping a child; it's not in the nav.) **(2) Playwright unblocked headless on this no-sudo WSL2 box** — `tests/e2e/playwright.config.ts` now auto-detects `~/.pw-deps/extracted/...` chromium libs → `LD_LIBRARY_PATH` + `--no-sandbox` (see the updated step-4 in "Testing — E2E" below; one-time `apt-get download` recipe there). **(3) 9 parent/auth/RTL specs reconciled** (P1-01/03/04/11/12-FE, rtl-alignment-polish, rtl-reverify-fresh, parent-final-capture, parent-lang-check; 87 stale assertions fixed) to the SHIPPED app — these were failing on STALE spec expectations, not app bugs: removed all `html[dir]`/`documentElement.dir` asserts (app intentionally never sets `<html dir>`; RTL is component-level `writingDirection`/Arabic-copy/position), replaced post-register `waitForURL(/add-child/)` with the two-step inline wizard's `onboarding-add-child-tile` on `/register`, dropped the removed `add-child-country` field, retargeted `overview-header`→`parent-header`/`overview-root`, and swapped role-by-text/`getByRole` selectors for named testIDs (`register-*`, `locale-switch-<loc>`, `role-card-*`). All 1242 specs parse. **Zero genuine app bugs left** per the 9-agent reconcile pass. **Next:** run E2E in controlled batches (Metro degrades after ~60 sequential API-heavy tests → restart Expo between spec files, or `--workers` once seeded). QC (`qc-test-designer`) intentionally paused per lead.
...
- **⚠️ Sandbox/WSL e2e run recipe (load-bearing — without these the harness can't start):**
  ...
  4. **Chromium system libs without root (NOW AUTOMATED in `playwright.config.ts`):** no passwordless sudo, so `playwright install-deps` can't run. The headless shell only needs 3 libs (Ubuntu 24.04 renamed `libasound2`→`libasound2t64`). One-time userspace fetch:
     ```
     cd /tmp && apt-get download libnspr4 libnss3 libasound2t64 \
       && for d in *.deb; do dpkg -x "$d" ~/.pw-deps/extracted; done
     ```
     `tests/e2e/playwright.config.ts` then **auto-detects** `~/.pw-deps/extracted/usr/lib/x86_64-linux-gnu` and prepends it to `LD_LIBRARY_PATH`, and sets `launchOptions.args=['--no-sandbox']` (WSL2 has no user namespaces). So once `~/.pw-deps` exists, `playwright test` just works — no env vars to export. (If you ever skip the config, the manual equivalent is `export LD_LIBRARY_PATH="$HOME/.pw-deps/extracted/usr/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH"` + `--no-sandbox`.)
