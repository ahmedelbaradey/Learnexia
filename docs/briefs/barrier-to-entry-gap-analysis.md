...
1. **Build-order inversion.** ✅ **RESOLVED (2026-05-23).** The phases were resequenced so **Gamification = Phase 3 (Week 5)** builds *before* **AI Tutor = Phase 4 (Weeks 6–7)** — aligning with the doc's "habit loop before AI" order. (Story IDs kept stable: Gamification stays `P4-xx`, AI Tutor stays `P3-xx`; see [user-stories/README.md](../../user-stories/README.md).) *Original finding: the doc's order is gamified quiz → curriculum intelligence → student tracking → adaptive → AI → behavioral; the prior phasing built AI tutoring before gamification — the exact ordering the doc warns "is why many AI projects fail." Curriculum intelligence remains post-MVP (mitigated by the hand-authored MVP slice, P2-11).*
