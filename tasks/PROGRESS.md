...
- **Wave 9:** P2-09-BE (home dashboard — `GET /api/Learning/Dashboard` + `GetDashboardQuery` + `DashboardController` + 4 DTOs + new `GetMostRecentActivitySubjectIdAsync` repo method + 11 integration tests; Phase-2 zero-state for XP/Streak/Mission/League) — ready for PR on feat/P2-09-home-dashboard
- **Wave 9:** P2-05-BE (open + complete a lesson — `AddLessonContent` migration + rewritten `GetLessonQueryHandler` + `[HttpGet("{id:int}")]` route + 4 seeded demo lessons + 11 integration tests including full e2e) — open PR #66
