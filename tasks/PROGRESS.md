...
- **2026-06-06 — FE status reconciliation:** board corrected against `main` ground truth — Phase-1 FE (P1-01/02/03/04) and Phase-2 student FE (P2-05/06/07/09, merged via PR #70/#71/#72/#74) flipped 🔲→✅; **P8-04 FE corrected ✅→🔲 (branch `feat/P8-04` was backend-only — no FE shipped)**. Open-WIP FE branches: `feat/P4-08-gamification-screens-motion` (resumable), `feat/design-system-pixel-align` (stale, holds font/RTL fixes).
- **P8-04 (BE only):** Change a child's learning language (parent-only, fresh start) — backend merged; **parent FE not built** (carry-forward).
