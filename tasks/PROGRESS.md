...
- **Wave 11 (BE+FE):** P4-04 (lose hearts and enter Practice Mode — Phase 3 Gamification: AddHeartsColumnsAndHeartLoss migration on StudentXpProfiles (Hearts/LastHeartRefillAtUtc) + new HeartLosses audit table (CreationAuditedEntity, OriginEventId unique idempotency, LessonId/SkillId for forensic traceability) + LazyHeartRefiller pure static with UTC-normalized math (regression-tested) + StudentXpProfile mutation methods (LoseHeart/RegainHeartFromPractice/RefreshHeartsAgainst, [NotMapped] InPracticeMode) + LoseHeartCommand + handler (out-of-order guard + idempotency + row-lock + audit + HeartsDepletedDomainEvent) + RegainHeartFromPracticeCommand (practice-your-way-out regen) + AnswerSubmittedIntegrationEventHandler extended (wrong→LoseHeart, correct→AwardXp+Regen, each in own try/catch) + Practice Mode gates on 3 reward handlers (suspend XP/streak when Hearts=0, lessons still attemptable) + IStudentHeartsQuery cross-module seam wired into Learning dashboard + DashboardDto extended (Hearts + InPracticeMode) + HeartsOptions config + FE bundle (dashboard hearts wire + Practice Mode pill + EN/AR i18n) + 11 unit tests + 14 integration tests; lead-approved Cap=5/30min/regen-on-correct/no-block-lessons/FE-bundled) — open as PR on feat/P4-04-hearts-practice-mode
