...
function AppShell({ active, onNav, children }) {
  ...
  return (
    <div className="pd-shell" style={{ display: 'flex', height: 820, background: 'var(--pd-canvas,#0F172A)', color: '#F8FAFC', ...appFont }}>
      <PDSidebar active={active} onChange={onNav}/>
      <div className="pd-main" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {children}
        <nav className="pd-bottombar" style={{
          position: 'sticky', bottom: 0, zIndex: 50, height: 62,
          ...
          {tabs.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => onNav(t.id)} style={{
                ...
                color: on ? '#A5B4FC' : '#64748B', fontWeight: 700, fontSize: 10, fontFamily: 'inherit', padding: '4px 8px',
              }}>
                <span style={{ fontSize: 20, filter: on ? 'none' : 'grayscale(0.6) opacity(0.7)' }}>{t.icon}</span>
                {t.label}
              </button>
            );
          })}
