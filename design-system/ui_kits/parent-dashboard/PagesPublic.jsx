...
// ────────────────────────────────────────────────────────────── LOGIN (web)
function LoginWebPage({ onLogin, onRegister, onLanding, onBack, role = 'parent' }) {
  ...
  return (
    <div style={{ minHeight: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#0F172A', color: '#F8FAFC', ...webFont }}>
      ...
      <div style={{
        ...
        padding: 'clamp(22px, 5vw, 56px)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
      }}>
        {[...Array(12)].map((_, i) => (
          <div key={i} style={{
      ...
      {/* Right form */}
      <div style={{ padding: 'clamp(22px, 5vw, 56px)', display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 520, margin: '0 auto', width: '100%' }}>
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ margin: 0, fontSize: 14, color: '#94A3B8' }}>{role === 'parent' ? "Log in to follow your children's progress" : 'Log in to keep your streak alive 🔥'}</p>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#1E293B', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', marginBottom: 18 }}>
          <span style={{ fontSize: 20 }}>{role === 'parent' ? '👨‍👩‍👦' : '🎓'}</span>
          <span style={{ flex: 1, fontWeight: 700, fontSize: 14, color: '#F8FAFC' }}>Signing in as {role === 'parent' ? 'Parent' : 'Student'}</span>
          <button onClick={onBack || onLanding} style={{ ...textBtn(), fontSize: 13 }}>Change</button>
        </div>
...
function RegisterWebPage({ onRegister, onLogin, onLanding }) {
  ...
  return (
    <div style={{ minHeight: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#0F172A', color: '#F8FAFC', ...webFont }}>
      <div style={{ padding: 'clamp(22px, 5vw, 56px)', display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 560, margin: '0 auto', width: '100%' }}>
      ...
      <div style={{
        position: 'relative', overflow: 'hidden',
        background: 'linear-gradient(165deg,#1E1B4B 0%,#3B2C8F 50%,#5B21B6 100%)',
        padding: 'clamp(22px, 5vw, 56px)', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 28,
      }}>
...
// ────────────────────────────────────────────────────────────── SPLASH (web)
function SplashWebPage({ onContinue }) {
  return (
    <div onClick={onContinue} style={{
      width: '100%', minHeight: 760, position: 'relative', cursor: 'pointer', overflow: 'hidden',
      background: 'radial-gradient(circle at 50% 42%,#4F3FB0 0%,#3B2C8F 38%,#241B6A 100%)',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', ...webFont,
    }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 22, position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontWeight: 900, fontSize: 48, color: '#F8FAFC', letterSpacing: '-0.02em' }}>Learnexia</div>
          <div style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', marginTop: 10, fontWeight: 500 }}>Parent Portal</div>
        </div>
        <div style={{ width: 260, height: 6, borderRadius: 9999, background: 'rgba(0,0,0,0.35)', overflow: 'hidden', marginTop: 8 }}>
          <div style={{ height: '100%', width: '55%', background: 'linear-gradient(90deg,#C4B5FD,#818CF8)', borderRadius: 9999 }}/>
        </div>
...
// ────────────────────────────────────────────────────────────── ROLE SELECT (web)
function RoleSelectWebPage({ onPick }) {
  const roles = [
    { id: 'parent',  emoji: '👨‍👩‍👦', label: 'Parent', sub: "Track your children's progress, reports and energy" },
    { id: 'student', emoji: '🎓',       label: 'Student', sub: 'Learn, play and level up' },
  ];
  return (
    <div style={{ width: '100%', minHeight: 760, background: '#0F172A', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', ...webFont }}>
      <div style={{ position: 'absolute', top: -160, left: '50%', transform: 'translateX(-50%)', width: 620, height: 420, borderRadius: '50%', background: 'radial-gradient(circle, rgba(168,85,247,0.28) 0%, rgba(168,85,247,0) 65%)', pointerEvents: 'none' }}/>
      <div style={{ position: 'relative', width: 460, display: 'flex', flexDirection: 'column', gap: 26, padding: 24 }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 44, marginBottom: 6 }}>🎮</div>
          <div style={{ fontWeight: 900, fontSize: 30, color: '#F8FAFC' }}>Welcome to Learnexia</div>
          <div style={{ fontSize: 15, color: '#94A3B8', marginTop: 8 }}>Who's signing in?</div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {roles.map(r => (
            <button key={r.id} onClick={() => onPick(r.id)} style={{
              display: 'flex', alignItems: 'center', gap: 18, padding: 22, borderRadius: 22,
            ...
            onPointerOut={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.background = '#1E293B'; }}>
              <div style={{ width: 64, height: 64, borderRadius: 18, background: 'rgba(79,70,229,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>{r.emoji}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 19 }}>{r.label}</div>
                <div style={{ fontSize: 13, color: '#94A3B8', marginTop: 3 }}>{r.sub}</div>
              </div>
              <div style={{ color: '#A5B4FC', fontSize: 24 }}>›</div>
            </button>
          ))}
        </div>
        <div style={{ textAlign: 'center', fontSize: 13, color: '#64748B', lineHeight: 1.5 }}>Children log in with the email a parent assigned — they never create their own account.</div>
      </div>
    </div>
  );
}
...
Object.assign(window, {
  LandingPage, LoginWebPage, RegisterWebPage, SplashWebPage, RoleSelectWebPage,
