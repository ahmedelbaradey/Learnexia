...
- `DashboardComponents.jsx` — shared building blocks: `PDSidebar`, `PDHeader`, `PDStatCard`, `PDActivityChart`, `PDWeakAreas`, `PDPanel`, `PDRecommendation`, plus `PDChildSwitcher` (compact pill in header / full in sidebar) and `PDAccountMenu` (header avatar → Language · Theme · Log out).
- `PagesPublic.jsx` — pre-auth surfaces: `SplashWebPage`, `RoleSelectWebPage`, `LandingPage`, `LoginWebPage`, `RegisterWebPage`.
...
### Public (pre-auth)
0. **Splash → Role Select** — `SplashWebPage` (branded 🌟 loading screen) then `RoleSelectWebPage` (👨‍👩‍👦 Parent / 🎓 Student). Picking a role goes to Login.
...
2. **Login** — Split layout. Left: purple visual panel with brand + tagline. Right: form with a read-only **"Signing in as Parent · Change"** role badge (not a toggle), email + password, remember me, forgot, Google / Apple / Microsoft social buttons. Panel padding is `clamp(22px,5vw,56px)` so inputs don't touch the edge.
...
- Responsive via **container queries on `.frame`** (desktop / tablet / mobile); the bottom tab bar lives inside `.frame`.
