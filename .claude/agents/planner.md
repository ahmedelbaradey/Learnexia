...
3. **Assign each task to an agent** — `db-migration`, `backend-feature`, or `frontend` (and note where `reviewer` gates). **If the story has a UI surface, insert a `designer` stage before the frontend batch** (Design Spec → frontend implements it); mark backend-only stories "no design stage." **If the story exposes HTTP endpoints, insert an `api-tester` stage after `backend-feature`** (runtime integration tests); mark stories with no HTTP surface "no api-tester." **If the story is security-sensitive (auth/authz, user or child data, file upload, AI prompts, secrets, payments), insert a `security-auditor` stage before the reviewer gate.**
