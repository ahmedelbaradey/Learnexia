...
- **HARD RULE — every agreed decision becomes stories + tasks, every session; but ASK FIRST before generating them.** Any scope/decision agreed with the lead in discussion MUST be written up as **user story file(s)** (`user-stories/<Phase>/<ID>-<slug>.md`) **and per-stack task files** (`tasks/Backend/...` **and** `tasks/Frontend/...` — **both FE and BE**), listed in the two READMEs, **before it is implemented**. **The agent does NOT auto-generate them:** first **ASK the lead** — propose the story breakdown (IDs / titles / FE-vs-BE scope split) and get explicit approval — **then** author the stories + tasks, then run the pipeline (analyzer → planner → …). Decisions do **not** live only in chat, but the lead controls *when* and *how* a decision is decomposed into stories. Binds **every session and every agent** — no build starts on an agreement that isn't first recorded as a story + tasks, and no story/task generation starts without the lead's go-ahead.
...
9. **Agreed decisions → stories + tasks (ASK FIRST, every session).** Any scope agreed with the lead MUST become user story file(s) **and** per-stack task files (Backend **and** Frontend), listed in the READMEs, **before** implementation — see **Work intake**. But **ask the lead before generating them** — propose the breakdown (IDs / titles / FE-vs-BE scope), get approval, then author; **never spin up story/task generation unilaterally**. Decisions don't live only in chat. Binds every session and every agent.
